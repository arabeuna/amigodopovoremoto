...
# Configurar encoding UTF-8 para o sistema
import locale
try:
...
from config.config_database import db_config, is_executable
from database_local import local_db
...
if hasattr(app, 'jinja_env'):
    app.jinja_env.globals['unicode'] = str

# Configurar resposta HTTP com UTF-8
@app.after_request
...
# Inicializar sistema de banco de dados (MongoDB ou SQLite)
print(f"🔧 Modo executável: {is_executable()}")
print(f"🗄️ Tipo de banco: {db_config.get_database_type()}")

if db_config.is_local_database():
    print("📁 Usando banco de dados local SQLite")
    # Inicializar dados padrão no SQLite se necessário
    try:
        # Verificar se há dados no banco local
        if local_db.contar_alunos() == 0:
            print("📦 Inicializando dados padrão no banco local...")
            # Aqui você pode adicionar dados iniciais se necessário
        print("✅ Banco de dados SQLite inicializado com sucesso!")
        mongodb = None  # Não usar MongoDB
...
# Funções auxiliares para abstração de banco de dados
def get_database_adapter():
    """Retorna o adaptador de banco de dados apropriado"""
    if db_config.is_local_database():
        return local_db
    else:
        return get_db_integration()

def listar_alunos_db():
    """Lista alunos do banco de dados ativo"""
    if db_config.is_local_database():
        return local_db.listar_alunos()
    else:
        try:
            aluno_dao = AlunoDAO()
            return aluno_dao.listar_todos()
        except Exception as e:
            print(f"Erro ao listar alunos MongoDB: {e}")
            return []

def inserir_aluno_db(aluno_data):
    """Insere aluno no banco de dados ativo"""
    if db_config.is_local_database():
        return local_db.inserir_aluno(aluno_data)
    else:
        try:
            aluno_dao = AlunoDAO()
            return aluno_dao.inserir(aluno_data)
        except Exception as e:
            print(f"Erro ao inserir aluno MongoDB: {e}")
            return None

def buscar_aluno_por_id_db(aluno_id):
    """Busca aluno por ID no banco de dados ativo"""
    if db_config.is_local_database():
        return local_db.buscar_aluno_por_id(aluno_id)
    else:
        try:
            aluno_dao = AlunoDAO()
            return aluno_dao.buscar_por_id(aluno_id)
        except Exception as e:
            print(f"Erro ao buscar aluno MongoDB: {e}")
            return None

def atualizar_aluno_db(aluno_id, aluno_data):
    """Atualiza aluno no banco de dados ativo"""
    if db_config.is_local_database():
        return local_db.atualizar_aluno(aluno_id, aluno_data)
    else:
        try:
            aluno_dao = AlunoDAO()
            return aluno_dao.atualizar(aluno_id, aluno_data)
        except Exception as e:
            print(f"Erro ao atualizar aluno MongoDB: {e}")
            return False

def listar_atividades_db():
    """Lista atividades do banco de dados ativo"""
    if db_config.is_local_database():
        return local_db.listar_atividades()
    else:
        try:
            atividade_dao = AtividadeDAO()
            return atividade_dao.listar_todas()
        except Exception as e:
            print(f"Erro ao listar atividades MongoDB: {e}")
            return []

def contar_alunos_db():
    """Conta alunos no banco de dados ativo"""
    if db_config.is_local_database():
        return local_db.contar_alunos()
    else:
        try:
            aluno_dao = AlunoDAO()
            return aluno_dao.contar_total()
        except Exception as e:
            print(f"Erro ao contar alunos MongoDB: {e}")
            return 0
...
class SistemaAcademia:
    def __init__(self):
        ...
        self.arquivo_dados = 'data/dados_alunos.json'
        self.arquivo_atividades = 'data/atividades_sistema.json'
        self.arquivo_turmas = 'data/turmas_sistema.json'
...
def cadastrar_aluno():
    try:
        ...
        # Processar upload de foto 3x4
        foto_filename = None
        if 'foto' in request.files:
            foto = request.files['foto']
            if foto and foto.filename != '' and allowed_image_file(foto.filename):
                ...
                foto_path = os.path.join(app.config['FOTOS_FOLDER'], foto_filename)
                try:
                    foto.save(foto_path)
